...
```python
# 回测模式 - 禁用实时数据，避免网络依赖
strategy = TradingStrategy(config, symbol='BTC/USDT', enable_live_data=False)
# 资金费率和多空比评分将返回0，不影响其他技术指标
```

**实盘时**：
```python
# 实盘模式 - 启用实时数据
strategy = TradingStrategy(config, symbol='BTC/USDT', enable_live_data=True)
# 将实时获取资金费率和多空比数据
```

**为什么要这样设计？**
- 回测应该使用历史数据，而不是运行时的实时数据
- 实时API调用会导致回测结果不可重现
- 避免回测时的网络依赖和API限流问题
...
| 移动止损 | ✅ | ✅ | 基于价格变化 |
| 分批止盈 | ✅ | ✅ | 基于盈利百分比 |
| 资金费率监控 | ❌ | ✅ | 需要实时API，回测时返回0分 |
| 多空比监控 | ❌ | ✅ | 需要实时API，回测时返回0分 |
